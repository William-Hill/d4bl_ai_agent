...
class D4BLResearchAnalyzer:
    ...
    def _save_combined_results(self, results: Dict) -> str:
        """Save the combined research and analysis results in both JSON and markdown formats"""
        timestamp = results["timestamp"]
        query_part = results["query"][:50].lower().replace(" ", "_")
        
        # Create directories if they don't exist
        os.makedirs('research_analysis', exist_ok=True)
        
        # Save JSON version
        json_filename = f"research_analysis/data_{query_part}_{timestamp}.json"
        with open(json_filename, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        
        # Save markdown version
        md_filename = f"research_analysis/report_{query_part}_{timestamp}.md"
        
        # Create markdown content
        md_content = f"""# Research Analysis: {results['query']}
...
## Research Findings
{self._format_research_findings(results.get('research', {}))}

## Analysis
### Key Points
{self._format_list(results.get('analysis', {}).get('analysis', {}).get('key_points', []))}

### Main Themes
{self._format_list(results.get('analysis', {}).get('analysis', {}).get('main_themes', []))}

### Data Quality Assessment
{self._format_data_quality(results.get('analysis', {}).get('analysis', {}).get('data_quality', {}))}

### Recommendations
{self._format_list(results.get('analysis', {}).get('analysis', {}).get('recommendations', []))}

## Sources
{self._format_sources(results.get('research', {}).get('sources', []))}
...
"""
    ...
    def _format_research_findings(self, research: Dict) -> str:
        """Format research findings for markdown"""
        if not research:
            return "*No research findings available*"
        
        findings = research.get('results', '')
    ...
    def _format_data_quality(self, quality: Dict) -> str:
        """Format data quality metrics for markdown"""
        if not quality:
            return "*No data quality metrics available*"
        
        return '\n'.join([
            f"- **{metric.title()}:** {score:.2f}"

# Training Script Extraction — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the interactive Colab training notebook (`notebooks/training/d4bl_fine_tuning.py`) into a headless CLI script (`scripts/training/train.py`) with argparse, phase-level checkpointing, progress output, training health checks, and a post-training report.

**Architecture:** Single-file script with top-level functions for each phase. A custom `TrainerCallback` captures per-step telemetry. Phase completion is detected by checking for output directories. Health checks are heuristic (no LLM). Report is generated as Markdown + JSON.

**Tech Stack:** Python, argparse, Unsloth, TRL (SFTConfig/SFTTrainer), transformers (TrainerCallback), torch, datasets

**Spec:** `docs/superpowers/specs/2026-03-28-training-script-extraction-design.md`

**Source notebook:** `notebooks/training/d4bl_fine_tuning.py`
...
```
scripts/training/
├── train.py              # NEW — the headless CLI training script (this plan)
└── config.py             # EXISTS — shared constants (no changes needed)
```

Output artifacts (written to `--output-dir` at runtime, not committed):
```
{output_dir}/
├── domain_merged/           # Phase 1 merged checkpoint
├── adapter_parser/          # Phase 2a adapter weights
├── adapter_explainer/       # Phase 2b adapter weights
├── adapter_evaluator/       # Phase 2c adapter weights
├── gguf/                    # Phase 3 GGUF exports
│   ├── d4bl-query-parser-q4_k_m/
│   ├── d4bl-explainer-q4_k_m/
│   └── d4bl-evaluator-q4_k_m/
├── phase1_checkpoints/      # Intermediate training checkpoints
├── parser_checkpoints/
├── explainer_checkpoints/
├── evaluator_checkpoints/
├── training_report.md       # Human-readable report
└── training_telemetry.json  # Machine-readable metrics
```

No tests in this plan — the script requires a GPU to run. Validation comes from the training report and the existing `test_training/test_integration_models.py` which validates GGUF outputs after registration.

---

## Task 1: Script Skeleton — Imports, Args, Main Orchestrator

**Files:**
- Create: `scripts/training/train.py`

- [ ] **Step 1: Create the script with imports and argparse**
...
Run: `python -c "import ast; ast.parse(open('scripts/training/train.py').read()); print('OK')"`
Expected: `OK` (no import errors since we're only checking syntax, not runtime imports)

- [ ] **Step 3: Commit**

```bash
git add scripts/training/train.py
git commit -m "feat(training): add train.py skeleton with argparse and main orchestrator (#138)"
```

---

## Task 2: Utility Functions — Validation, Data Loading, Helpers

**Files:**
- Modify: `scripts/training/train.py`

These functions go between the constants block and `parse_args`.

- [ ] **Step 1: Add the utility functions**

Insert after the `ADAPTER_CONFIGS` dict and before `parse_args`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add validation, data loading, and utility functions (#138)"
```

---

## Task 3: Telemetry Callback

**Files:**
- Modify: `scripts/training/train.py`

- [ ] **Step 1: Add the TelemetryCallback class**

Insert after the `free_vram` function, before `parse_args`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add TelemetryCallback for step-level metric capture (#138)"
```

---

## Task 4: Phase 1 — Domain Adaptation Training

**Files:**
- Modify: `scripts/training/train.py`

- [ ] **Step 1: Add the `train_domain_adapter` function**

Insert after the `TelemetryCallback` class, before `parse_args`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add Phase 1 domain adaptation training (#138)"
```

---

## Task 5: Phase 2 — Task-Specific Adapter Training

**Files:**
- Modify: `scripts/training/train.py`

- [ ] **Step 1: Add the `train_task_adapter` function**

Insert after `train_domain_adapter`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add Phase 2 task-specific adapter training (#138)"
```

---

## Task 6: Phase 3 — GGUF Export

**Files:**
- Modify: `scripts/training/train.py`

- [ ] **Step 1: Add the `export_gguf` function**

Insert after `train_task_adapter`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add Phase 3 GGUF export (#138)"
```
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add training health checks with heuristic analysis (#138)"
```
...
- [ ] **Step 1: Add the `generate_report` function**

Insert after `print_health_checks`:
...
```bash
git add scripts/training/train.py
git commit -m "feat(training): add training report and telemetry generation (#138)"
```

---

## Task 9: Final Assembly and Verification

**Files:**
- Modify: `scripts/training/train.py` (ensure correct ordering of all functions)

- [ ] **Step 1: Verify the complete script parses cleanly**
...
Run: `python scripts/training/train.py --help`

Expected output should show:
```
usage: train.py [-h] [--data-dir DATA_DIR] [--output-dir OUTPUT_DIR]
                [--phases PHASES] [--model MODEL] [--quantize QUANTIZE]
                [--force]

D4BL Training Pipeline — headless LoRA fine-tuning
...
```

Note: This will fail if run outside a CUDA environment (the top-level `from unsloth import FastLanguageModel` requires CUDA). That's expected — the script is designed for GPU environments. To verify argparse in isolation, run:

```bash
python -c "
import ast, sys
tree = ast.parse(open('scripts/training/train.py').read())
# Check that main, parse_args, and all phase functions exist
func_names = [node.name for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.ClassDef))]
expected = ['parse_args', 'main', 'validate_data_dir', 'check_phase_complete',
            'load_training_data', 'train_domain_adapter', 'train_task_adapter',
            'export_gguf', 'run_health_checks', 'print_health_checks',
            'generate_report', 'TelemetryCallback']
missing = [f for f in expected if f not in func_names]
if missing:
    print(f'MISSING: {missing}', file=sys.stderr)
    sys.exit(1)
print(f'OK — {len(func_names)} functions/classes found')
"
```

Expected: `OK — 17 functions/classes found` (12 expected + 5 private helpers)

- [ ] **Step 3: Verify the complete function ordering is correct**

The file should have this structure (top to bottom):
1. Module docstring
2. Imports
3. Constants (`ALL_PHASES`, `REQUIRED_DATA_FILES`, `ADAPTER_CONFIGS`)
4. Private helpers (`_get_hf_token`, `_format_duration`, `_now_utc`)
5. Print helpers (`print_banner`, `print_completion_banner`)
6. Validation and data (`validate_data_dir`, `check_phase_complete`, `load_jsonl`, `load_dataset_from_jsonl`, `format_and_tokenize`, `load_training_data`, `free_vram`)
7. `TelemetryCallback` class
8. Training phases (`train_domain_adapter`, `train_task_adapter`)
9. Export (`export_gguf`)
10. Health checks (`run_health_checks`, `print_health_checks`)
11. Report (`generate_report`)
12. CLI (`parse_args`, `main`)
13. `if __name__ == "__main__": main()`

If any function is out of order (e.g., `main` defined before `train_domain_adapter`), reorder to match this layout.

- [ ] **Step 4: Final commit**

```bash
git add scripts/training/train.py
git commit -m "feat(training): complete headless training script extraction (#138)"
```
